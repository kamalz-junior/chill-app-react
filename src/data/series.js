export const tedLasso = {
  title: "Ted Lasso - Season 1",
  releaseDate: "August 14, 2020",
  platform: "Apple TV+",
  episodeCount: 10,
  episodeRuntime: "Approximately 30 minutes",
  genres: ["Comedy", "Drama", "Sports"],
  creators: ["Jason Sudeikis", "Bill Lawrence", "Brendan Hunt", "Joe Kelly"],
  developedBy:
    "Based on a character created by Jason Sudeikis for NBC Sports promotional videos",
  executiveProducers: [
    "Jason Sudeikis",
    "Bill Lawrence",
    "Brendan Hunt",
    "Joe Kelly",
    "Jeff Ingold",
    "Liza Katzer",
  ],
  mainCast: {
    "Jason Sudeikis": "Ted Lasso",
    "Hannah Waddingham": "Rebecca Welton",
    "Brendan Hunt": "Coach Beard",
    "Jeremy Swift": "Leslie Higgins",
    "Phil Dunster": "Jamie Tartt",
    "Brett Goldstein": "Roy Kent",
    "Juno Temple": "Keeley Jones",
    "Nick Mohammed": "Nathan 'Nate' Shelley",
  },
  synopsis:
    "Ted Lasso, an optimistic American college football coach from Kansas, is unexpectedly hired to manage AFC Richmond, a struggling English Premier League soccer team. Despite his lack of soccer experience, Ted’s folksy charm, relentless positivity, and unconventional leadership begin to win over the skeptical team, its owner Rebecca Welton—who secretly hopes he’ll fail as revenge against her ex-husband—and the fans. Over the course of the season, Ted navigates cultural differences, personal challenges, and team dynamics to inspire change both on and off the pitch.",
  setting: "London, United Kingdom (fictional AFC Richmond soccer club)",
  language: "English",
  awards:
    "Nominated for 20 Primetime Emmy Awards for its first season, winning 7, including Outstanding Comedy Series (2021)",
  notableThemes: [
    "Optimism",
    "Teamwork",
    "Redemption",
    "Personal Growth",
    "Mental Health",
  ],
  episodeReleaseSchedule: {
    initialRelease: {
      episodes: [1, 2, 3],
      date: "August 14, 2020",
    },
    weeklyReleases: {
      episodes: [4, 5, 6, 7, 8, 9, 10],
      startDate: "August 21, 2020",
      endDate: "October 2, 2020",
    },
  },
  productionCompanies: [
    "Warner Bros. Television",
    "Doozer Productions",
    "Universal Television",
  ],
  episodes: [
    {
      episodeNumber: 1,
      title: "Pilot",
      img: "src/assets/images/ted-lasso-1.png",
      releaseDate: "August 14, 2020",
      description:
        "Ted arrives in London to coach AFC Richmond, facing hostility from fans and players.",
    },
    {
      episodeNumber: 2,
      title: "Biscuits",
      img: "src/assets/images/ted-lasso-2.png",
      releaseDate: "August 14, 2020",
      description:
        "Ted bonds with Rebecca through homemade biscuits while adjusting to his new role.",
    },
    {
      episodeNumber: 3,
      title: "Trent Crimm: The Independent",
      img: "src/assets/images/ted-lasso-3.png",
      releaseDate: "August 14, 2020",
      description:
        "A cynical journalist shadows Ted for a day, leading to unexpected insights.",
    },
    {
      episodeNumber: 4,
      title: "For the Children",
      img: "src/assets/images/ted-lasso-4.png",
      releaseDate: "August 21, 2020",
      description:
        "Ted orchestrates a reconciliation between Roy and Jamie at a charity gala.",
    },
    {
      episodeNumber: 5,
      title: "Tan Lines",
      img: "src/assets/images/ted-lasso-5.png",
      releaseDate: "August 28, 2020",
      description:
        "Ted’s wife and son visit, prompting tough decisions during a crucial match.",
    },
    {
      episodeNumber: 6,
      title: "Two Aces",
      img: "src/assets/images/ted-lasso-0.png",
      releaseDate: "September 4, 2020",
      description:
        "Ted recruits Dani Rojas to replace an uncooperative Jamie, but a curse strikes the team.",
    },
    {
      episodeNumber: 7,
      title: "Make Rebecca Great Again",
      releaseDate: "September 11, 2020",
      description:
        "Rebecca celebrates her divorce anniversary as the team faces a match in Liverpool.",
    },
    {
      episodeNumber: 8,
      title: "The Diamond Dogs",
      releaseDate: "September 18, 2020",
      description:
        "Ted and Roy seek relationship advice from unlikely sources.",
    },
    {
      episodeNumber: 9,
      title: "All Apologies",
      releaseDate: "September 25, 2020",
      description:
        "Pressure mounts to bench Roy, and Rebecca reveals her true intentions to Ted.",
    },
    {
      episodeNumber: 10,
      title: "The Hope That Kills You",
      releaseDate: "October 2, 2020",
      description:
        "Richmond plays a decisive match that determines Ted’s fate with the club.",
    },
  ],
};
